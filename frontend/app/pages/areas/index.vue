<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Scientific Areas</h1>
        <p class="text-gray-500 text-sm">Manage areas like 'Computer Science', 'Mathematics', etc.</p>
      </div>
    ...
    <UModal v-model:open="isModalOpen" title="New Scientific Area">
      <template #content>
        <UCard>
          <template #header>
            <div class="flex items-center justify-between">
              <h3 class="font-semibold">Create Area</h3>
          ...
          <UForm :schema="schema" :state="state" @submit="handleSave" class="space-y-4">
            <UFormField label="Name" name="name">
              <UInput v-model="state.name" placeholder="e.g. Computer Science" class="w-full" autofocus />
            </UFormField>
...
const handleSave = async () => {
  saving.value = true
  try {
    await refStore.createArea(state.name)
    toast.add({ title: 'Area created', color: 'green' })
