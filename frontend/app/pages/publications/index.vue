  ...
  <div class="flex gap-6 items-start relative">
    <div class="flex-1 min-w-0 space-y-6">
      ...
      <div class="flex items-center justify-between">
        ...
        <UButton icon="i-heroicons-cloud-arrow-up" color="primary" label="Upload" to="/publications/create" />
      </div>
      <div
        class="flex gap-3 items-center bg-white dark:bg-gray-900 p-3 rounded-lg border border-gray-200 dark:border-gray-800 shadow-sm">
        <UIcon name="i-heroicons-funnel" class="w-5 h-5 text-gray-400" />

        <!-- Search Input -->
        <UInput v-model="search" icon="i-heroicons-magnifying-glass" placeholder="Search titles..." size="sm"
          class="flex-1 min-w-[200px]" />

        <!-- Hidden Status Filter (Admins/Resp only) -->
        <USelectMenu v-if="authStore.isAdmin || authStore.isResponsavel" v-model="pubHiddenFilter"
          :items="pubFilterOptions" option-attribute="label" placeholder="Visibility" class="w-40" size="sm"
          :searchable="false">
          <template #label>
        ...
        <USelectMenu v-model="selectedTags" :items="tagOptions" placeholder="Filter by Tags" multiple searchable
          class="w-48" size="sm">
          <template #label>
        ...
        <!-- Clear Filters -->
        <UButton v-if="selectedTags.length > 0 || search !== ''" icon="i-heroicons-x-mark" color="gray" variant="ghost"
          size="xs" @click="clearFilters" />
      </div>
      <!-- Table -->
      <UCard :ui="{ body: { padding: '' } }">
        <UTable :data="filteredPublications" :columns="columns" :loading="pubStore.loading">
          <template #titulo-cell="{ row }">
            <NuxtLink :to="`/publications/${row.original.id}`"
          ...
          <template #tags-cell="{ row }">
            <div class="flex flex-wrap gap-1">
              <UBadge v-for="tag in row.original.tags.slice(0, 3)" :key="tag" color="primary" variant="subtle"
                size="md">
                {{ tag }}
              </UBadge>
          ...
          <template #actions-cell="{ row }">
            <UTooltip text="Download File">
              <UButton icon="i-heroicons-arrow-down-tray" size="xs" color="primary" variant="ghost"
                :loading="downloadingId === row.original.id" @click="handleDownload(row.original)" />
            </UTooltip>
            <UTooltip text="View Details">
              <UButton icon="i-heroicons-eye" size="xs" color="primary" variant="ghost"
                @click="openSidePanel(row.original)" />
            </UTooltip>
            <UTooltip text="Histórico de edições">
              <UButton icon="i-heroicons-clock" size="xs" color="primary" variant="ghost"
                @click="navigateTo(`/publications/${row.original.id}/history`)" />
            </UTooltip>
          </template>
    ...
    <aside v-if="isPanelOpen" class="w-96 flex-shrink-0 sticky top-0 h-[calc(100vh-5rem)] overflow-hidden">
      <UCard class="h-full flex flex-col" :ui="{ body: { padding: 'p-0', base: 'flex-1 flex flex-col min-h-0' } }">
        ...
        <div
          ...
          <div class="flex items-center gap-2">
            <UTooltip text="Abrir publicação">
              <UButton color="gray" variant="ghost" icon="i-heroicons-arrow-top-right-on-square" size="xs"
                :disabled="!selectedPub?.id" @click="goToPublication" />
            </UTooltip>
            <UButton color="gray" variant="ghost" icon="i-heroicons-x-mark-20-solid" size="xs"
            ...
            <div class="p-4 space-y-6">
                ...
                <div class="flex flex-col items-end gap-1.5">
                  ...
                  <div class="flex items-center gap-1" @mouseleave="hoverRating = 0">
                    ...
                    <UTooltip text="Remove Rating" v-if="userRatingValue > 0">
                      <UButton icon="i-heroicons-x-mark" size="2xs" color="red" variant="ghost"
                        :loading="ratingUpdating" @click="handleClearRating" />
                    </UTooltip>
                  </div>
                </div>
              ...
              <div>
                <div class="flex justify-between items-center mb-3">
                  ...
                  <!-- Download-->
                  <UButton label="Download" icon="i-heroicons-arrow-down-tray" size="xs" color="primary" variant="soft"
                    :loading="downloadingId === selectedPub?.id" @click="handleDownload(selectedPub)" />
                </div>
                <dl class="space-y-3 text-sm">
                  ...
                  <div class="pt-2">
                    ...
                    <dd class="flex flex-wrap gap-1">
                      <UBadge v-for="author in selectedPub?.autores" :key="author.username" color="gray"
                  ...
                  <div class="pt-2 flex justify-between items-center">
                    ...
                    <dd class="text-gray-900 dark:text-white font-medium flex items-center gap-2">
                      ...
                      <UTooltip v-if="canToggleVisibility" :text="selectedPub?.hidden ? 'Make Visible' : 'Hide'">
                        <UButton :icon="selectedPub?.hidden ? 'i-heroicons-eye' : 'i-heroicons-eye-slash'" size="md"
                          color="gray" variant="ghost" @click="togglePubVisibility" :loading="visibilityUpdating" />
                      </UTooltip>
                    </dd>
                  </div>
                </dl>
            ...
            <div class="flex flex-col h-full">
              ...
              <div v-if="authStore.isAdmin || authStore.isResponsavel"
                ...
                <USelectMenu v-model="commentsFilter" :items="filterOptions" option-attribute="label"
                  placeholder="Filter comments..." size="sm" :searchable="false" @change="refreshComments" />
              </div>
              ...
              <div class="p-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/20">
                <UForm :state="{ text: newComment }" @submit="submitComment">
                  ...
                  <div class="flex justify-end">
                    <UButton type="submit" label="Post Comment" size="xs" color="primary" :loading="submittingComment"
              ...
              <div class="p-4 space-y-4 flex-1 overflow-y-auto">
                ...
                <div v-else v-for="comment in sortedComments" :key="comment.id"
                  ...
                  <div class="absolute top-2 right-2">
                    <UDropdownMenu v-if="getCommentActions(comment).length > 0" :items="getCommentActions(comment)"
                      :content="{ align: 'end' }">
                      <UButton color="gray" variant="ghost" icon="i-heroicons-ellipsis-vertical" size="2xs"
          ...
          <UCard>
            ...
            <UForm :state="{ motive: visibilityMotive }" @submit="confirmVisibilityChange" class="space-y-4">
              ...
              <UFormField label="Motive" name="motive" :required="!commentToToggle?.hidden">
                <UTextarea v-model="visibilityMotive" placeholder="e.g. Inappropriate content..." :rows="2" autofocus />
              </UFormField>

              <div class="flex justify-end gap-3 pt-2">
                <UButton label="Cancel" color="neutral" variant="ghost" @click="isVisibilityModalOpen = false" />
                <UButton type="submit" :label="commentToToggle?.hidden ? 'Show' : 'Hide'"
                  :color="commentToToggle?.hidden ? 'green' : 'red'" :loading="visibilityUpdating" />
              </div>
            </UForm>
          </UCard>
        </template>
...
const goToPublication = async () => {
  if (!selectedPub.value?.id) return
  await navigateTo(`/publications/${selectedPub.value.id}`)
}
